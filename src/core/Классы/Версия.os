...
Процедура Прочитать(Знач ВерсияСтрокой)
	...
	Иначе
		ОшибкаЧтенияВерсии = СтрШаблон("Основная версия <%1> должна содержать только цифры", Токен.Значение);
		Возврат;
	КонецЕсли;

	// Чтение второстепенной
	Токен = Парсер.Следующий();

	Если Токен.Тип = Парсер.ТипКонецТекста Тогда
		Возврат;
	ИначеЕсли Токен.Значение <> "." Тогда
		ОшибкаЧтенияВерсии = СтрШаблон("Ожидается разделитель разрядов версии <.>, но найдено <%1>", Токен.Значение);
		Возврат;
	Иначе
		Токен = Парсер.Следующий();
	КонецЕсли;
	...
	Иначе
		ОшибкаЧтенияВерсии = СтрШаблон("Второстепенная версия <%1> должна содержать только цифры", Токен.Значение);
		Возврат;
	КонецЕсли;
	...
	Иначе
		ОшибкаЧтенияВерсии = СтрШаблон("Версия патча <%1> должна содержать только цифры", Токен.Значение);
		Возврат;
	КонецЕсли;
