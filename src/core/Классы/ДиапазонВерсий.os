...
Процедура ОбработатьПростоеСравнение(Знач ДиапазонСтрокой)
	...
	Лог.Отладка("Добавили сравнение: %1 знак: %2", ВерсияСравнения.ВСтроку(), Знак);
...
Процедура ОбработатьЛюбойСимвол(Знач ВерсияДиапазона)
	...
	Если ВерсияДиапазона = "*" Тогда
		...
		ДобавитьСравнение(Новый Версия("0.0.0"), ЛевыйЗнак);
		Возврат;
	...
	Если ИндексЛюбогоСимвола = 4 Тогда
		ДобавитьСравнение(Новый Версия(ВерсияДиапазона), ЛевыйЗнак);
		Возврат;
	КонецЕсли;
	...
	ДобавитьСравнение(Новый Версия(СтрокаЛевойВерсии), ЛевыйЗнак);
		
	ПраваяВерсия = Новый Версия(СтрокаЛевойВерсии);
	...
	ДобавитьСравнение(ПраваяВерсия, ПравыйЗнак);
...
Функция ДобавитьСравнение(Знач ВерсияСравнения, Знач ЗнакСравнения)

	ВерсияВСравнении = Сравнение[ЗнакСравнения];
